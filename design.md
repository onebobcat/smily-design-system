...
- Three breakpoints, not device-based guesses: **mobile** below 570px, **tablet** 570–1000px, **desktop** above 1000px. [L041]
- Primary navigation reflows per breakpoint — never a single "collapse to hamburger" mobile pattern. It's a three-step progression: full 256px labeled sidebar (desktop) → icon-only collapsed rail with no labels (tablet) → bottom tab bar with 5 icon+label items, overflow grouped under a trailing "Menu" tab (mobile). [L042]
- Page content containers cap at a **1126px max-width** rather than stretching to fill wide desktop viewports. The capped container stays left-aligned next to the sidebar — it does not center itself in the remaining space; extra viewport width becomes right-side margin, not extra content width. [L043]
- This is the same breakpoint logic the filter bar already follows (see §5): icon-only mobile trigger [L039] and the mobile "⋯" overflow ordering [L040] both key off the mobile threshold above.
...
*Sources: [`bridge-ds/knowledge-base/learnings.json`](bridge-ds/knowledge-base/learnings.json) (L001–L043), [CLAUDE.md](CLAUDE.md) design tokens, Smily Figma DS (`flMGdSqr1NNqKGK5jp9QtH`), Figma "Patterns -> responsiveness" (`Ubs0K3AhPSScgvN6qaRiuH`). Regenerate/extend this file the same way `CLAUDE.md`'s Design Rules section is kept in sync — new learnings should get a line here too, not just in the JSON.*
