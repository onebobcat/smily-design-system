...
- **filterBar.activeStateKnownIssue**: When filters are active, the 'Filters' button switches from outline to tonal (filled teal). This outline-to-tonal transition is inconsistent with other button state patterns and is tracked as a known issue pending a dedicated redesign — implement it as-is and do not introduce custom workarounds in the meantime.
- **filterBar.mobileIconOnlyTrigger**: On mobile, the filter trigger is icon-only: the funnel icon with no text label and no button border (desktop uses a bordered button with a "Filters" label and chevron). The chevron is dropped too, since tapping the trigger always opens a panel or bottom sheet, so it adds nothing on a touch surface. The icon-only trigger sits top-right of the screen header, adjacent to the primary CTA (e.g. + Create).
- **breakpoints.thresholds**: Smily designs for three breakpoints, not a generic device-based set: mobile below 570px, tablet from 570px to 1000px, and desktop above 1000px. Use these exact thresholds when deciding where a layout should reflow, not screen-size assumptions from a specific device.
- **navigation.responsiveBehavior**: Primary navigation adapts per breakpoint rather than collapsing to a single mobile pattern: the full 256px labeled sidebar on desktop (>1000px) becomes an icon-only collapsed rail with no labels on tablet (570-1000px), then a bottom tab bar with 5 icon+label items on mobile (<570px), with overflow items grouped under a trailing "Menu" tab. Never jump straight from the labeled sidebar to a hamburger menu.
- **layout.containerMaxWidth**: Page content containers cap at a maximum width (1126px) rather than stretching to fill wide desktop viewports. The capped container stays left-aligned next to the sidebar, not centered in the remaining space — extra viewport width becomes right-side margin, not extra content width.
...
### Calendar
- **filterBar.mobileOverflowOrder**: On mobile, when the toolbar already carries 4 or more elements (e.g. a multi-rental calendar with view tabs and date navigation), move the Filters and Legend triggers into a "more" (3-dot) overflow menu instead of keeping them inline. Order the overflow menu with view-density options before data-scoping controls: Compact view, Detailed view, Legend, Filters.
